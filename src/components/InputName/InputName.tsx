...
const InputName: React.FC<InputNameProps> = ({
    ...
    const handleChange = (index: number, value: string) => {
      ...
      const charIndex = inputIndexes[index];
      const isCorrect = value.toLowerCase() === name[charIndex].toLowerCase();
  
      if (isCorrect && index < inputIndexes.length - 1) {
        let nextIndex = index + 1;
        while (nextIndex < inputIndexes.length && name[inputIndexes[nextIndex]] === " ") {
          nextIndex++;
        }
        if (nextIndex < inputIndexes.length) {
          ...
          {name.split("").map((char, index) => {
            ...
            return (
              <div key={inputIndex} className={`${styles.inputItem} ${inputClass}`}>
                ...
                <input
                  ref={(el) => { if (el) inputRefs.current[inputIndex] = el; }}
                  type="text"
